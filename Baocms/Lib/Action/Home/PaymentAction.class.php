        ...
        if(!empty($detail)){
            if ($detail['type'] == 'ele') {
                ...
                $this->edu_success('恭喜您课程支付成功啦！', $detail);
              	...
              	//dump($detail);die;
              	//$detail['order_id']=1801121749351122;
              	//$money=(int)$detail['need_pay'];
              	//$relation=D('users_relation')->where(['user_id'=>$detail['user_id']])->find();
              	//if(!empty($relation['area_id'])){
              	//	$res=D('users')->where(['user_id'=>$relation['area_id']])->setInc('money',$money*0.1);
                  	
               // }
              //	if(!empty($relation['shop_id'])){
              	//	$res=D('users')->where(['user_id'=>$relation['shop_id']])->setInc('money',$money*0.2);
               // }
              	//if(!empty($relation['ditui_id'])){
              	//	$res=D('users')->where(['user_id'=>$relation['ditui_id']])->setInc('money',$money*0.3);
               // }
              	//if(!empty($relation['other_id'])){
              	//	$res=D('users')->where(['user_id'=>$relation['other_id']])->setInc('money',$money*0.1);
               // }
              	//dump($relation);die;
              	$return['status']=0;
              	$return['msg']='恭喜支付成功';
              	$this->ajaxReturn($return);
                $this->success('恭喜您支付成功了', U('members/index/index'));
