...
class  AppointAction extends CommonAction{
	...
	public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Appoint');
            if ($obj->add($data)) {
                $this->boSuccess('添加成功', U('Appoint/index'));
            }
            $this->boError('操作失败！');
        } else {
            $this->assign('cates', D('Appointcate')->fetchAll());
	 ...
	 private function createCheck() {
        $data = $this->checkFields($this->_post('data', false), $this->create_fields);
			...
			if (empty($data['end_date'])) {
				$this->boError('结束时间不能为空');
			}
			if (!isDate($data['end_date'])) {
				$this->boError('结束时间格式不正确');
			}
			$data['contents'] = SecurityEditorHtml($data['contents']);
			if (empty($data['contents'])) {
            $this->boError('家政内容不能为空');
			}
			if ($words = D('Sensitive')->checkWords($data['contents'])) {
				$this->boError('家政简介含有敏感词：' . $words);
			}
			$data['audit'] = 1;
        return $data;
    }
        ...
        if ($appoint_id = (int) $appoint_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $this->boSuccess('操作成功', U('appoint/index'));
                }
                $this->boError('操作失败');
            } else {
	...
	public function delete($appoint_id = 0) {
        if (is_numeric($appoint_id) && ($appoint_id = (int) $appoint_id)) {
            ...
            $this->boSuccess('删除成功！', U('appoint/index'));
        } else {
            ...
            if (is_array($appoint_id)) {
                ...
                $this->boSuccess('批量删除成功！', U('appoint/index'));
            }
            $this->boError('请选择要删除的预约项目');
        }
    }
	...
	public function audit($appoint_id = 0) {
        if (is_numeric($appoint_id) && ($appoint_id = (int) $appoint_id)) {
            ...
            $this->boSuccess('审核成功！', U('appoint/index'));
        } else {
            $appoint_id= $this->_post('appoint_id', false);
            if (is_array($appoint_id)) {
                $obj = D('Appoint');
		...
		if ($appoint_id = (int) $appoint_id) {
			...
			if ($data = $this->_post('data', false)) {
				...
				if (!empty($data['price']) && !empty($data['name']) && !empty($data['mobile']) && !empty($data['intro'])){
					...
					$this->boSuccess('操作成功', U('appoint/worker',array('appoint_id'=>$detail['appoint_id'])));
				}else{
					 $this->boError('内容不能为空');
				}
			}else{
	...
	public function worker_edit($worker_id){
		if ($worker_id = (int) $worker_id) {
			...
			if (!$worker = $Appointworker->find($worker_id)) {
                $this->boError('修改的内容不存在');
            }
			$appoint_id = $worker['appoint_id'];
            if (!$detail = $Appoint->find($appoint_id)) {
                $this->boError('请选择要设置的家政');
            }
            if ($detail['closed'] != 0) {
                $this->boError('该家政已被删除');
            }	
