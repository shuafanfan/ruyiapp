...
class UsersauxAction extends CommonAction{
    ...
    public function create(){
        if ($this->isPost()) {
            ...
            $obj = D('Usersaux');
            if ($obj->add($data)) {
                $this->baoSuccess('添加成功', U('usersaux/index'));
            }
            $this->baoError('操作失败！');
        } else {
            $this->display();
    ...
    public function edit($user_id = 0){
        if ($user_id = (int) $user_id) {
            ...
            if (!($detail = $obj->find($user_id))) {
                $this->baoError('请选择要编辑的会员认证');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                $data['user_id'] = $user_id;
                if (false !== $obj->save($data)) {
                    $this->baoSuccess('操作成功', U('usersaux/index'));
                }
                $this->baoError('操作失败');
            } else {
    ...
    public function delete($user_id = 0){
        if (is_numeric($user_id) && ($user_id = (int) $user_id)) {
            ...
            $this->baoSuccess('删除成功！', U('usersaux/index'));
        } else {
            ...
            if (is_array($user_id)) {
                ...
                $this->baoSuccess('批量删除成功！', U('usersaux/index'));
            }
            $this->baoError('请选择要删除的会员认证');
        }
    }
	//审核
            ...
            if($detail = D('Usersaux')->find($user_id)){
				...
				if(D('Users')->where(array('user_id'=>$user_id))->save($data)){
					if($obj->save(array('user_id' => $user_id, 'audit' => 1))){
						...
						$this->baoSuccess('审核成功！', U('usersaux/index'));
					}else{
						$this->baoError('更新审核信息失败');
					}
				}else{
					$this->baoError('更新会员表失败');
				}
			}else{
				$this->baoError('该认证信息不存在');
			}
        } else{
			$this->baoError('请选择你要审核的认证会员');
		}
    }
	//拒绝审核
