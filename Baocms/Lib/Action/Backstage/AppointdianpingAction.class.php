...
class AppointdianpingAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($dianping_id = $obj->add($data)) {
                ...
                $this->boSuccess('添加成功', U('Appointdianping/index'));
            }
            $this->boError('操作失败！');
        } else {
            $this->display();
    ...
    private function createCheck() {
        ...
        if (empty($data['order_id'])) {
            $this->boError('家政订单号不能为空');
        }
        if (!$order = D('Appointorder')->find($data['order_id'])) {
            $this->boError('家政订单不存在');
        }
        ...
        if ($dianping_id = (int) $dianping_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->boSuccess('操作成功', U('Appointdianping/index'));
                }
                $this->boError('操作失败');
            } else {
    ...
    private function editCheck() {
	 ...
	 public function delete($dianping_id = 0) {
        if (is_numeric($dianping_id) && ($dianping_id = (int) $dianping_id)) {
            ...
            $this->boSuccess('删除成功2！', U('Appointdianping/index'));
        } else {
            ...
            if (is_array($dianping_id)) {
                ...
                $this->boSuccess('删除成功！', U('Appointdianping/index'));
            }
            $this->boError('请选择要删除的设计师点评');
        }
    }
