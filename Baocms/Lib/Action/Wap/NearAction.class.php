...
class NearAction extends CommonAction{
    ...
    public function dingwei(){
        ...
        $lng = $_COOKIE['lng'];
        $nTime = time()+3600;
        if ($_COOKIE['localed'] != 2 || empty($lat) || empty($lng)) {
            $local = array($this->_param('lat'), $this->_param('lng'));
            setcookie('lat_ok', $local[0], $nTime,'/','.zscycn.com');
            setcookie('lng_ok', $local[1], $nTime,'/','.zscycn.com');
            setcookie('lat', $local[0], $nTime,'/','.zscycn.com');
            setcookie('lng', $local[1], $nTime,'/','.zscycn.com');
            $addr = $this->getArea($local[0], $local[1]);
            setcookie('addr', $addr, $nTime,'/','.zscycn.com');
            if (!empty($addr)) {
                setcookie('localed', 2, $nTime,'/','.zscycn.com');
            }
        }
        echo '1';
    }
	//首页城市定位  可定位至4级区域，区/县
		...
		$lng = $this->_param('lng');
        $nTime = time()+3600;
		setcookie('lat_ok', $lat, $nTime,'/','.zscycn.com');
		setcookie('lng_ok', $lng, $nTime,'/','.zscycn.com');
		setcookie('lat', $lat, $nTime,'/','.zscycn.com');
		setcookie('lng', $lng, $nTime,'/','.zscycn.com');
		...
		$mcityid = $_COOKIE['city_id'];
		$cityop = $_COOKIE['cityop'];
		if (!empty($mcityid)) {
