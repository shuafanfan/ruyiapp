...
class NewsAction extends CommonAction {
	protected function _initialize() {
        parent::_initialize();
		if(!$this->_CONFIG['operation']['news']) {
			$this->error('此功能已关闭');
			die;
		 }
        $this->assign('articlecates',$articlecates = D('Articlecate')->fetchAll());
    }
    ...
    public function index() {	
		...
		$this->assign('nextpage', LinkTo('news/load',$linkArr, array('t' => NOW_TIME, 'p' => '0000')));
	 ...
	 public function load() {
        ...
        switch ($order) {
            case '1':
                ...
                break;
            case '2':
                $orderby = array('zan' => 'asc');
                $linkArr['order'] = $order;
                break;
            case '3':
                $orderby = array('donate_num' => 'asc');
                $linkArr['order'] = $order;
                break;
            case '4':
                ...
                break;
            default:
                $orderby = array('create_time' => 'desc', 'article_id' => 'desc');
                break;
        }
		$this->assign('order', $order);
		...
		foreach ($list as $k => $val) {
			$count = $obj ->get_article_pic_count($val['article_id']);//获取统计数量后赋值
            $list[$k]['count'] = $count;
        }
