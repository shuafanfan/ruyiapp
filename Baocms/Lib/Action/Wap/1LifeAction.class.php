...
class LifeAction extends CommonAction{
    ...
    public function fujin() {
        ...
        $this->assign('news', $news = D('Article')->where(array('city_id'=>$this->city_id, 'closed' => 0, 'audit' => 1))->order(array('create_time' => 'desc'))->limit(0, 10)->select());



         $this->assign('ele', $ele = D('Ele')->where(array('city_id'=>$this->city_id, 'audit' => 1))->order(array('orderby' => 'desc'))->limit(0, 10)->select());



        $this->assign('life', $life = D('Life')->where(array('city_id'=>$this->city_id, 'closed' => 0, 'audit' => 1))->order(array('create_time' => 'desc'))->limit(0, 10)->select());
        //当前经纬度获取
	...
	public function subscribe(){
		...
		$detail = D('Life')->find($life_id);
        if(!$detail = D('Life')->find($life_id)) {
			$this->fengmiMsg('信息不存在');
        }
		if(false == D('Life')->subscribeLife($life_id,$this->uid)){
			 $this->fengmiMsg(D('Life')->getError());
        }else{
			$this->fengmiMsg('恭喜您订阅成功', U('life/detail', array('life_id' => $detail['life_id'])));
		}
    }
}