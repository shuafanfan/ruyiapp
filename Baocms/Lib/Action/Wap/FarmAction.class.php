        ...
        if (empty($this->uid)) {
            ...
            if(!$data['gotime']){
                ...
                if($add = D('FarmOrder')->add($data)){
                    $this->fengmiMsg('下单成功',U('farm/pay',array('order_id'=>$add)));
                }else{
                    $this->fengmiMsg('下单失败!');
                }
            }
    ...
    public function pay2(){
     ...
     public function favorites() {
        ...
        if (!$detail = D('Farm')->where(array('farm_id'=>$farm_id))->find()) {
            $this->fengmiMsg('没有该农家');
        }
        if ($detail['closed']) {
            $this->fengmiMsg('该商家已经被删除');
        }
        ...
        if (D('Shopfavorites')->add($data)) {
            $this->fengmiMsg('恭喜您收藏成功！', U('farm/detail', array('farm_id' => $farm_id)));
        }
        $this->fengmiMsg('收藏失败！');
    }
