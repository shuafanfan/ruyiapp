...
class RunningModel extends CommonModel{
	...
	//跑腿抢单
	public function Running_Confirm_Qiang($running_id,$cid){
		$running = D('Running')->find($running_id);
		if(!empty($running)){
	...
	public function Running_Confirm_Complete($running_id,$cid){
		...
		if(!empty($running)){
			if (false !== D('Running')->save( array('running_id' => $running_id,'status' => 3,'end_time' => NOW_TIME))){
                    D('Sms')->sms_Running_Delivery_User($running_id);//给买家通知配送状态
                    $info = '跑腿订单ID：'.$running_id.'结算，实际支付：'.round($running['need_pay']/100,2).'元';
					if ($running['need_pay'] > 0) {
                        D('Runningmoney')->add(array(
						   'running_id' => $running_id, 
						   'delivery_id' => $running['cid'], 
						   'user_id' => $running['cid'], 
						   'money' => $running['need_pay'], 
						   'type' => running, 
						   'create_time' => NOW_TIME, 
						   'create_ip' => get_client_ip(), 
						   'intro' => $info
					));
                       D('Users')->addMoney($running['cid'], $running['need_pay'],$info);  //写入配送员余额
                    }
                    return true;
		 		}
		}else{
